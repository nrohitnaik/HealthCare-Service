...
public class UserConsoleHelper {
		...
		if (initialSelction == 1) {
			...
			if (returnValue.getStatus() == "valid") {
				...
				case "doctor": {
					...
					int doctorRegistrationId = Math
							.toIntExact(doctorService.fetchDoctorByUserId(returnValue.getId()).getRegistrationId());
					doctorConsoleHelper.validatedDoctorMenu(returnValue.getUserName(), returnValue.getId(),
	...
	public void signUpUser(String role) {
		...
		// call userService and returns the userName if valid userName or else
	...
	/**
	 * validates user has entered valid password or not login validation and
	 * providing respective pages based on the role of the user
	 * 
	 * @return User
	 */
