...
public class InventoryConsoleHelper {
	...
	private void getDoctorList() {
		...
		System.out.println(
				"Registration Id \t\t Doctor's Name \t\t Specialization \t\t Department \t\t email\t\t phone number\t\t status");
		doctorList.forEach(a -> {
			String availablity = "No";
			if (a.isAvailability() == 1) {
				availablity = "Yes";
			}
			System.out.println(a.getRegistrationId() + "\t\t" + a.getFirstName() + " " + a.getLastName() + "\t\t"
					+ a.getSpecialization() + "\t\t" + a.getDepartment() + "\t\t" + a.getEmail() + "\t\t"
					+ a.getPhoneNumber() + "\t\t" + availablity);
		});
		System.out.println("Press any key to return to previous menu..");
		inventoryScanner.nextLine(); // swallow the input and call the inventory
										// menu
		inventoryManagerMenu();
	}
	...
	private void getBillingMenu() {
		...
		if (0 != billsToBePopultated.size()) {
			for (Bill aBill : billsToBePopultated) {
				System.out.println("Sr.No \t\t Diagnosis Id \t\t Patient Name\t\t  \t\t Doctor Name \t\t admitted date "
						+ "\t\t discharged date \t\t prescription");

				System.out.println(count + "\t\t" + aBill.getDiagnosisId() + "\t\t" + aBill.getPatientName() + "\t\t "
						+ aBill.getDoctorName() + "\t\t" + aBill.getAdmitDate() + "\t\t" + aBill.getDischargeDate()
	...
	private void getInventoryManagmentMenu() {
		...
		for (Inventory aInventory : inventoryList) {
			System.out.println(count + "\t\t" + aInventory.getName() + "\t\t" + aInventory.getDescription() + "\t\t"
					+ aInventory.getQuantity() + "\t\t" + aInventory.getVendorContact() + "\t\t"
					+ aInventory.getType());
			count++;
		}

		System.out.println(
				"Select the \"Sr.No\" to update the details. \nSelect '0' to add new Inventory. \n Enter 'back' to return to previous menu");
		// inventoryScanner.nextLine();
		...
		if (checkStringIsInterger(selecteInventoryId)) {
			...
			if (intValueOfInput != 0) {
				...
				// TODO need to handle indexoutofbound if userchoose any other
				// int ..providing relavent message and other option
			} else {
	...
	private void newInventoryForm() {
		...
		System.out.println(inventory.getName()
				+ "  details has been saved successfully. Press any key to return to to previous menu");
	...
	private void getFormForUpdatingInventory(Inventory inventory) {
		...
		System.out.println(inventory.getName()
				+ "  details has been updated successfully. Press any key to return to to previous menu");
		inventoryScanner.nextLine(); // swallow the input
		inventoryManagerMenu();
